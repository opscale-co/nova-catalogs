...
**Project:** nova-catalogs
**Project Type:** package
**Module Prefix:** catalogs
**Tenant Aware:** no
**Created:** 2026-05-01
...
**Reason `nova-catalogs` is not tenant-aware:**
This is a standalone, reusable Laravel Nova package distributed via Packagist for managing
catalogs (lookup tables, taxonomies, configurable lists). Tenant isolation is the
responsibility of the consuming application — the package exposes plain Eloquent models
and Nova resources that the host app may scope with its own multi-tenancy strategy
(e.g. global scopes, polymorphic `catalogable` morphs). Hard-coding `tenant_id` would
break installations in single-tenant apps and leak tenancy concerns into a generic package.
